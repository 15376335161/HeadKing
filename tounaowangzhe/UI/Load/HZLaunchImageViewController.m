    ...
    if(!self.showDefaultImage){
        ...
        [RACObserve(self, model) subscribeNext:^(id x) {
            if (self.model.imageUrl) {
                [self.launchImageView sd_setImageWithURL:[NSURL URLWithString:self.model.imageUrl] placeholderImage:[UIImage imageNamed:@"闪屏"] completed:^(UIImage * _Nullable image, NSError * _Nullable error, SDImageCacheType cacheType, NSURL * _Nullable imageURL) {
