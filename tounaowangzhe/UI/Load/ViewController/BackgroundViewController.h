...
//消息未读数
@property (nonatomic,copy) NSString * _Nullable messageUnReadCount;

- (void)showLoginViewController:(LoginViewController *_Nullable)loginVC;
- (void)showLoginViewController:(LoginViewController *_Nullable)loginVC animated:(BOOL)animated completion:(void (^ __nullable)(void))completion;
///** 当前用户是否登录 */
//- (BOOL)isUserLogin;
//- (void)autoLogin;// 自动登录
