...
- (NSString *)AES256EncryptWithKeyString:(NSString *)key
{
    ...
    NSData *decry = [self aes256_encrypt:key];
...
{
    if(self){
        NSData *decry = [self aes256_decrypt:key];
        NSString *descryStr = [[NSString alloc] initWithData:decry encoding:NSUTF8StringEncoding];
        return descryStr;
    }
    else{
...
+(NSString *) aes256_encrypt:(NSString *)key Encrypttext:(NSString *)text
{
    ...
    //对数据进行加密
    NSData *result = [NSData AES256ParmEncryptWithKey:key Encrypttext:data];
    //转换为2进制字符串
...
+(NSString *) aes256_decrypt:(NSString *)key Decrypttext:(NSString *)text
{
    ...
    //对数据进行解密
    NSData* result = [NSData  AES256ParmDecryptWithKey:key Decrypttext:data];
