...
+ (NSString *)thousandFoUnit:(NSString *)number {
    NSString *retuerstr = number;
    if ([number integerValue]/10000 >= 1) {
        retuerstr = [NSString stringWithFormat:@"%ld万元",[number integerValue]/10000];
    }
    return retuerstr;
}


+ (void)haveNewVersion:(UIViewController *)VC {
    HttpRequestMode *model = [[HttpRequestMode alloc]init];
    model.name= @"获取更新信息";
    model.url = GetApkUpdate;
    NSMutableDictionary *params = [NSMutableDictionary dictionary];
    NSString *version = [kAppBuildVersion stringByReplacingOccurrencesOfString:@"." withString:@""];
    [params setObject:version forKey:@"apk_version"];
    [params addUnEmptyString:@"default_channel_ios" forKey:@"channel_key"];
    [params addUnEmptyString:[[NSBundle mainBundle] objectForInfoDictionaryKey:@"CFBundleDisplayName"] forKey:@"package_name"];
...
//        YMUpdateView *updateView = [[YMUpdateView alloc] initWithTitle:nil imgStr:@"发现新版本" message:[NSString stringWithFormat:@"%@", [response.result objectForKey:@"msg"]] sureBtn:@"立即更新" cancleBtn:@"取消"];
...
+ (BOOL)isNeedShowGuideView {
    //老Version
    NSString *defaultVersion = [UserDefaultsTool getStringWithKey:@"AppVersion"];
    NSString *currentVersion = [[[NSBundle mainBundle] infoDictionary] objectForKey:@"CFBundleShortVersionString"];
    if([defaultVersion isEqualToString:@""] || [defaultVersion compare:currentVersion options:NSNumericSearch] == NSOrderedAscending){
        return YES;
    }
    return NO;
}
