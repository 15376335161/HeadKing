...
+ (CGSize )comressSizeByImage:(UIImage* )image {
    ...
    CGFloat zoomHeight = kScreenWidth * WHrate;
    
    CGSize zoomSize;
    if (image.size.width >= kScreenWidth ||
        image.size.height >= kScreenHeight) {
        zoomSize = CGSizeMake(kScreenWidth, zoomHeight);
    }else{
        zoomSize = image.size;
    }
    NSLog(@"zoomSize == %f heigh = %f",zoomSize.width,zoomSize.height);
    return zoomSize;
}
